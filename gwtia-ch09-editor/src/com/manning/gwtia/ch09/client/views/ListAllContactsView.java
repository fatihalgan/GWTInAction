...
import com.manning.gwtia.ch09.client.views.ListAllContactsEditor.ListAllContactEditorUiBinder;

public class ListAllContactsView extends Composite {

	Logger log = Logger.getLogger("");

	interface ListAllContactEditorUiBinder extends UiBinder<Widget, ListAllContactsView> {
	}
	
	interface Driver extends RequestFactoryEditorDriver<List<ContactProxy>, HasDataEditor<ContactProxy>> {
	}

	private static ListAllContactEditorUiBinder uiBinder = GWT.create(ListAllContactEditorUiBinder.class);
	...
	void onClickDeleteAllContacts(ClickEvent e) {
		...
		requestFactory.createContactRequest().findAllContacts().to(receiver).fire();
	}
}
