...
/**
 * Renders a simple interface for getting a specified user's Titter history. The
 * interface has the user to type in the target user's Twitter name into a text
 * box and then clicking the "Get Tweets" button will display the user's history
 * on the page.
 */
public class GTwitter extends Composite {
	...
	public GTwitter() {
		...
		/*
		 * An async callback coded as an anonymous class. This can easily be
		 * moved into an inner class or a separate class, making it easier to
		 * test as a separate unit.
		 * 
		 * This callback handles the results from a call to the
		 * getUserTimeline() method on the server.
		 */
		final AsyncCallback<ArrayList<FeedData>> updateTweetPanelCallback = new AsyncCallback<ArrayList<FeedData>>() {

			/*
			 * Displays an alert pop-up message when the service call returns an
			 * error.
			 */
			...
			/*
			 * Populates the tweetPanel with the tweets retrieved from the
			 * server.
			 */
			public void onSuccess(ArrayList<FeedData> results) {
				...
				for (FeedData status : results) {
					...
					String dateStr = DateTimeFormat.getFormat(fmt).format(
							status.getCreatedAt());
					tweetPanel
							.add(new Label(dateStr + ": " + status.getText()));
				}
			}
		};

		/*
		 * An click handler coded as an anonymous class. This can easily be
		 * moved into an inner class or a separate class, making it easier to
		 * test as a separate unit.
		 * 
		 * This method triggers a call to getUserTimeline() on the server when a
		 * user clicks the btnGetTweets button.
		 */
		btnGetTweets.addClickHandler(new ClickHandler() {
			public void onClick(ClickEvent event) {
				// Mentioned in the book as useful for testing if your server
				// isn't sending
				...
				xsrf.getNewXsrfToken(new AsyncCallback<XsrfToken>() {
					...
					public void onSuccess(XsrfToken token) {
						TwitterServiceAsync service = GWT
								.create(TwitterService.class);
						((HasRpcToken) service).setRpcToken(token);
						/*
						 * Alternative way to set the target URL on the server.
						 * In most cases (as we do here), you would use the
						 * @RemoteServiceRelativePath annotation on the service
						 * interface.
						 */
						// ((ServiceDefTarget)
