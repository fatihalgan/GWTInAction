...
public class CellListExamples extends Composite {
	...
	private static CellTypesUiBinder uiBinder = GWT.create(CellTypesUiBinder.class);
	...
	private void setUpSelectionModel() {
		final MultiSelectionModel<PhotoDetails> selectionModel = 
			new MultiSelectionModel<PhotoDetails>();
		...
		selectionModel.addSelectionChangeHandler(new SelectionChangeEvent.Handler() {
			...
			public void onSelectionChange(SelectionChangeEvent event) {
				if (selectionModel.getSelectedSet().size() > 5) {
					...
					selectionModel.setSelected(selectionModel.getSelectedSet().iterator().next(), 
						false);
				}
			}
		});
