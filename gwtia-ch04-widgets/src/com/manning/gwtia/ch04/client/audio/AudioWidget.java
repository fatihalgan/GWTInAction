...
public class AudioWidget extends Composite {
  ...
  public void onValueChange(ValueChangeEvent<Boolean> evt) {
	  ...
	  if (src.equals(loop)) audio.setLoop(evt.getValue());
	  if (src.equals(autoplay)) audio.setAutoplay(evt.getValue());
  }
  ...
  public void onChangeSpeed(ValueChangeEvent<Boolean> evt) {
	  if (audio == null) return;
	  Object src = evt.getSource();
	  if (src.equals(normal)) audio.setPlaybackRate(1.0);
	  else if (src.equals(dble)) audio.setPlaybackRate(2.0);
	  else if (src.equals(half)) audio.setPlaybackRate(0.5);
  }
