...
public class LiveSearchPanel extends SamplePanel {
	...
	final LiveSearchServiceAsync liveSearchService = (LiveSearchServiceAsync)GWT.create(LiveSearchService.class);
		...
		searchText.addKeyboardListener(new KeyboardListener() {
			...
			public void onKeyUp(Widget sender, char keyCode, int modifiers) {
				...
				if (searchText.getText().length() > 0) {
					AsyncCallback callback = new AsyncCallback() {
						public void onSuccess(Object result) {
							...
							for (Iterator iter = resultItems.iterator(); iter.hasNext();) {
								liveResultsPanel.setText(row++, 0, (String) iter.next());
							}
						}
					...
					liveSearchService.getCompletionItems(searchText.getText(), callback);
				}
			}
		});
